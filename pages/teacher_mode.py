...
# --- 페이지 설정 및 CSS (생략 없이 원본 CSS 넣으세요) ---
st.set_page_config(
    page_title="AI 튜터 모드",
    page_icon="👨‍🏫",
    layout="wide",
...
def main():
    ...
    # ... 이하, 기존의 칼럼/버튼/칠판/음성/저장 등 레이아웃 100% 동일하게 유지

    # 칠판 영역 등 기존과 동일 (단, 칠판 부분에만 아래처럼 콤보 추가)
    ...
    # ... 이하 기존 마이크/텍스트 입력/저장/메모 등 전부 동일하게 삽입

    # [핵심!] 새 응답이 생성될 때마다 아래 컴포넌트가 뜨도록 수정
    ...
    try:
        if user_input:
            ...
            if ai_response and "오류가 발생했습니다" not in ai_response:
                ...
                st.session_state.latest_ai_response = ai_response   # [NEW] 칠판+TTS+타이핑+EQ 콤보용 저장
